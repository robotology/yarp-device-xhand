...
#include <xhand-multithread-control-test/xhand.h>
...
int main(int argc, char** argv)
{
    ...
    std::vector<double> th_1_durations, th_2_durations{};
    const int num_tests{1000000};
    th_1_durations.reserve(num_tests);
    th_2_durations.reserve(num_tests);

    std::cout << "Starting test of " << num_tests << " calls..." << std::endl;


    // Run th_1_fun and th_2_fun concurrently in separate threads.
    ...
    std::thread th_1(th_1_fun);
    std::thread th_2(th_2_fun);
    th_1.join();
    th_2.join();
    ...
    if (failed.load(std::memory_order_relaxed)) {
     ...
     std::string filename = "xhand-multithread-control-test.csv";
     results_file.open(filename);
     results_file << "Control1Time,Control2Time\n";
     std::cout << "\n\nControl1Time\tControl2Time:";

    // write rows up to the minimum available samples for each vector
    size_t rows = std::min(th_1_durations.size(), th_2_durations.size());
    for (size_t i = 0; i < rows; ++i) {
        results_file << th_1_durations[i] << "," << th_2_durations[i] << "\n";
        std::cout << "\nDuration " << i << ": " << th_1_durations[i] << " ns" << "\t" << th_2_durations[i] << " ns";
    }
     results_file.close();
