...
xhand_control::ErrorStruct xHAND::sendCommand(const HandCommand_t& command){
    return m_ctrl.send_command(m_id, command);
}
...
bool xHAND::test(bool readFirst, bool force_update, const std::vector<float>& finger_positions){
    ...
    if(read_time.size() >= test_size){
        ...
        std::string filename = "xhand-joint-test-" + std::string(readFirst ? "read" : "control") + "-first.csv";
